...
Profiling data can be captured and visualized with [flamegraph](https://github.com/brendangregg/FlameGraph).
Steps:
- download `flamegraph`
- execute `valida run` with env variable `PROFILING_PATH` set to an absolute path where traces will be stored
```bash
PROFILING_PATH="$(pwd)/profiling.txt" valida run ...
```
- process the collected traces with `flamegraph` (if `flamegraph.pl` script is not in PATH use a path to it)
```bash
flamegraph.pl profiling.txt > profiling.svg
```
- open `profiling.svg` in a browser. That graphical file is interactive and works only when opened in a browser.

### Built-in profiler

A built-in profiler is available by compiling valida-vm with

```
cargo build --release --features print-timers
```

Here is an example output for `valida run`
![](./media/profiling_valida_run.png)

Here is an example output for `valida prove`
![](./media/profiling_valida_prove.gif)
