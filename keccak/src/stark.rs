use super::columns::WrapKeccakCols;
use super::KeccakFChip;
use crate::columns::NUM_WRAP_KECCAK_COLS;
...
impl<F> BaseAir<F> for KeccakFChip {
    fn width(&self) -> usize {
        NUM_WRAP_KECCAK_COLS
    }
}

impl<AB> Air<AB> for KeccakFChip
where
    AB: AirBuilder,
    ...
    AB::Var: Debug,
{
    fn eval(&self, builder: &mut AB) {
        fn xor_expr<E: AbstractField>(x: E, y: E) -> E {
            x.clone() + y.clone() - x * y.double()
        }

        fn xor3_expr<E: AbstractField>(x: E, y: E, z: E) -> E {
            xor_expr(x, xor_expr(y, z))
        }

        fn andn_expr<E: AbstractField>(x: E, y: E) -> E {
            (E::one() - x) * y
        }

        let main = builder.main();

        let local: &WrapKeccakCols<AB::Var> = main.row_slice(0).borrow();
        let next: &WrapKeccakCols<AB::Var> = main.row_slice(1).borrow();

        let not_final_step = AB::Expr::one() - local.step_flags[NUM_ROUNDS - 1];
        ...
        for y in 0..5 {
            for x in 0..5 {
                let get_bit = |z| {
                    ...
                    xor3_expr(a_prime.into(), c.into(), c_prime.into())
                };
        ...
        for y in 0..5 {
            for x in 0..5 {
                let get_bit = |z| {
                    ...
                    xor_expr(local.b(x, y, z).into(), andn)
                };
        ...
        let get_xored_bit = |i| {
            ...
            xor_expr(local.a_prime_prime_0_0_bits[i].into(), rc_bit_i)
        };
