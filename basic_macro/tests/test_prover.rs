...
fn prove_fibonacci() {
    ...
    let instance_data = machine.run(&rom, &mut FixedAdviceProvider::empty());
    ...
    let (pk, vk) = machine.pre_process(&config, show_traces.clone());
    let proof = machine.prove(
        &config,
        &pk,
        show_traces.clone(),
        show_traces.clone(),
    ...
    assert_eq!(
        machine.mem().read_bypass_logging(0x1000 + 4), // Return value
        Word::from_components_be([0, 1, 37, 17,])         // 25th fibonacci number (75025)
    );
}
