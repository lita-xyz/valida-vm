...
pub trait MachineWithBytesBus<F: Field>: Machine<F> {
...
pub trait MachineWithPersistentMemBus<F: Field>: Machine<F> {
    fn persistent_mem_bus(&self) -> BusArgument;
}

pub trait MachineWithPointerBus<F: Field>: Machine<F> {
