    ...
    AB::Var: Debug,
{
    fn eval(&self, builder: &mut AB) {
        ...
        builder
            ...
            .assert_eq(*local.input_1.index_be(0), *local.input_2.index_be(0));
        // case: top two bytes match
        builder
            .when_ne(local.byte_flag[0] + local.byte_flag[1], AB::Expr::one())
            .assert_eq(*local.input_1.index_be(1), *local.input_2.index_be(1));
        ...
        builder
            ...
            .assert_eq(*local.input_1.index_be(2), *local.input_2.index_be(2));
        // case: top four bytes match; must set z = 0
        builder
            .when_ne(flag_sum.clone(), AB::Expr::one())
            .assert_eq(*local.input_1.index_be(3), *local.input_2.index_be(3));
        builder
            .when_ne(flag_sum.clone(), AB::Expr::one())
        ...
        for i in 0..4 {
            builder.when(local.byte_flag[i]).assert_eq(
                AB::Expr::from_canonical_u32(256) + *local.input_1.index_be(i)
                    - *local.input_2.index_be(i),
                bit_comp.clone(),
            );
            // ensure that when the n-th byte flag is set, the n-th bytes are actually different
            builder.when(local.byte_flag[i]).assert_eq(
                (*local.input_1.index_be(i) - *local.input_2.index_be(i)) * local.diff_inv,
                AB::Expr::one(),
            );
        ...
        let top_comp_2: AB::Expr = local
            ...
            .map(|(bit, base)| bit * base)
            .sum();
        builder.assert_eq(top_comp_1, *local.input_1.index_be(0));
        builder.assert_eq(top_comp_2, *local.input_2.index_be(0));
