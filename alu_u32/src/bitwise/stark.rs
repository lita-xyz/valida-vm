...
impl<AB> Air<AB> for Bitwise32Chip
where
    AB: AirBuilder,
    ...
    AB::Var: Debug,
{
    fn eval(&self, builder: &mut AB) {
        ...
        for i in 0..MEMORY_CELL_BYTES {
            ...
            builder.assert_eq(*local.input_1.index_be(i), byte_1.clone());
            builder.assert_eq(*local.input_2.index_be(i), byte_2.clone());
            ...
            builder
                .when(local.is_and)
                .assert_eq(bitwise_and.clone(), *local.output.index_be(i));
            builder
                .when(local.is_or)
                .assert_eq(bitwise_or.clone(), *local.output.index_be(i));
            builder
                .when(local.is_xor)
                .assert_eq(bitwise_xor.clone(), *local.output.index_be(i));
